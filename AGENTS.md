...
The repo is the marketplace. `.claude-plugin/marketplace.json` declares every plugin and every skill path inline; `plugins/<name>/` holds one use case, with a `SKILL.md` per skill under `skills/` and bulk material in each skill's `references/`. Two things are worth knowing cold. Every `SKILL.md` under a plugin's `skills/` directory loads whether or not the plugin's `skills` array in `marketplace.json` lists it (verified at Claude Code v2.1.278; see `docs/plugin-mechanics.md`), so a stray directory ships and the array only matters for skills kept elsewhere. And an MCP facade parameter the server does not recognize can be dropped without complaint, so never hardcode a facade's parameters: `describe` the operation at runtime, then read every write back, because a success response is not evidence that the write landed.
